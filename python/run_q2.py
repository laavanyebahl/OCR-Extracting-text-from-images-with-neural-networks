...
delta1 = probs
delta1[np.arange(probs.shape[0]),y_idx] -= 1      # SAME AS BELOW
# delta1 = probs - y   SAME AS (y_hat - y)
        ...
        if len(val.shape) == 2:
            for i in range(val.shape[0]):
                for j in range(val.shape[1]):
                    ...
                    # loss
                    # be sure to add loss and accuracy to epoch totals
                    l_1, acc = compute_loss_and_acc(y, probs)
                    # params[k] = W1
                    ...
                    # loss
                    # be sure to add loss and accuracy to epoch totals
                    l_2, acc = compute_loss_and_acc(y, probs)

                    params['grad_'+k][i,j]= (l_1 - l_2)/(2*eps)
                    # params[k] = W2
            ...
            for i in range(val.shape[0]):
                ...
                params['grad_' + k][i] = (l_1 - l_2) / (2 * eps) *1e-6
                # params[k] = b2
...
for k in params.keys():
    if 'grad_' in k:
        # relative error
        err = np.abs(params[k] - params_orig[k])*1e-7/np.maximum(np.abs(params[k]),np.abs(params_orig[k]))
        err = err.sum()
